...
bool test(long double val){
	vector<halfplane> h;

	fr(i, n){
		point a = pol[i + 1] - pol[i];
		a = a.rot90();
		a = a/a.norm();
		a = a*val;

		h.pb(halfplane(pol[i] + a, pol[i + 1] + a));
	}

	return halfp_intersect(h);
}
...
signed main(){
	...
	while (cin>>n and n > 0){
		...
		long double lft = 0, rgt = INF;

		fr(i, 110){
			long double mid = (lft + rgt)/2;
