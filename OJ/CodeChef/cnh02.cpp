...
//Get area of a polygon, O(n)
coord get_area(vector<point> &P){
	if (P.size() < 3) 
		return 0;
	...
	for (int i = 0; i + 1 < P.size(); i++)
		area += P[i]^P[i + 1];
	return fabs(area + (P[P.size() - 1]^P[0]))/2;
}
...
signed main(){
	...
	fr(i, n){
		...
		fr(j, m)
			h.pb(halfplane(pol[i][j], pol[i][j + 1]));
	}

	vector<point> pnts = halfp_intersect(h);


	coord ans = get_area(pnts);
