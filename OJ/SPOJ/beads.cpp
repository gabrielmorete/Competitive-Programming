...
int32_t main(){
	...
	while (T--){
		...
		if (!ok){	
			...
			int st = 1;
			while (p[st] >= n) // primeiro cara que contem as duas strings
				st++;

			while (st < 2 * n and lcp[st] >= 2 * n - p[st]) // acha o menor indice valido (checa se o trecho importante está contido)
				st++;
			cout<<p[st] + 1<<endl;
		}
	}	
